document.addEventListener('DOMContentLoaded', () => {
    ...
    let state = {
        ...
        config: {
            ...
            // O token fornecido foi adicionado aqui como padrão inicial.
        ...
        try {
            ...
            if (configData) {
                ...
                state.importHistory = configData.importHistory || [];
                 ...
                 // Salva a configuração inicial se não houver nenhuma
    ...
    async function processData(records, source) {
        ...
        for (let i = 0; i < normalizedRecords.length; i++) {
            ...
            if (!newRecord.telefone) continue;
            ...
            if (confirm('Confirmação final: Todos os clientes e históricos de importação serão perdidos. Continuar?')) {
                ...
                try {
                    await new Promise((resolve, reject) => {
                        ...
                        tx.objectStore(CLIENTS_STORE).clear();
    ...
    function updateSelectionUI() {
        ...
        ui.selectAllPage.checked = state.filteredClients.length > 0 && getSelectedClientsOnPage().length === state.filteredClients.slice((state.currentPage - 1) * state.itemsPerPage, state.currentPage * state.itemsPerPage).length;
    ...
    function getSelectedClientsOnPage() {
        const start = (state.currentPage - 1) * state.itemsPerPage;
        const end = start + state.itemsPerPage;
        const pageClientIds = new Set(state.filteredClients.slice(start, end).map(c => c.id));
        return Array.from(state.selection).filter(id => pageClientIds.has(id));
    }
    ...
    function showToast(message, type = 'info') {
        ...
        toast.className = `p-4 rounded-lg shadow-lg text-white ${colors[type]} transform transition-all duration-300 ease-out opacity-0 translate-y-2`;
